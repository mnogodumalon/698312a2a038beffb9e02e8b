...
export default function TestfaellePage() {
  const [records, setRecords] = useState<Testfaelle[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Testfaelle | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Testfaelle | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Testfaelle | null>(null);
  const [sortKey, setSortKey] = useState('');
  const [sortDir, setSortDir] = useState<'asc' | 'desc'>('asc');
  const [testprojekteList, setTestprojekteList] = useState<Testprojekte[]>([]);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const [mainData, testprojekteData] = await Promise.all([
        LivingAppsService.getTestfaelle(),
        LivingAppsService.getTestprojekte(),
      ]);
      setRecords(mainData);
      setTestprojekteList(testprojekteData);
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Testfaelle['fields']) {
    await LivingAppsService.createTestfaelleEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Testfaelle['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateTestfaelleEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteTestfaelleEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      title="Testfälle"
      subtitle={`${records.length} Testfälle im System`}
      action={
    ...
    >
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('erwartetes_ergebnis')}>
                <span className="inline-flex items-center gap-1">
                  Erwartetes Ergebnis
                  {sortKey === 'erwartetes_ergebnis' ? (sortDir === 'asc' ? <ArrowUp size={14} /> : <ArrowDown size={14} />) : <ArrowUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{getTestprojekteDisplayName(record.fields.projekt)}</span></TableCell>
                ...
                <TableCell className="max-w-xs"><span className="truncate block">{record.fields.erwartetes_ergebnis ?? '—'}</span></TableCell>
                <TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={6} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Testfälle. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
      ...
      <TestfaelleDialog
        ...
        testprojekteList={testprojekteList}
