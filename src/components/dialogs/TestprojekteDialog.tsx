...
interface TestprojekteDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Testprojekte['fields']) => Promise<void>;
  defaultValues?: Testprojekte['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function TestprojekteDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = false, enablePhotoLocation = true }: TestprojekteDialogProps) {
  const [fields, setFields] = useState<Partial<Testprojekte['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'testprojekte');
      await onSubmit(clean as Testprojekte['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      const schema = `{\n  "projektname": string | null, // Projektname\n  "beschreibung": string | null, // Beschreibung\n  "startdatum": string | null, // YYYY-MM-DD\n  "status": LookupValue | null, // Status (select one key: "geplant" | "in_bearbeitung" | "abgeschlossen" | "pausiert") mapping: geplant=Geplant, in_bearbeitung=In Bearbeitung, abgeschlossen=Abgeschlossen, pausiert=Pausiert\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Testprojekte['fields']>;
      });
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Testprojekte bearbeiten' : 'Testprojekte hinzufügen';
