...
export class LivingAppsService {
  ...
  static async getTestprojekte(): Promise<Testprojekte[]> {
    ...
    return enrichLookupFields(records, 'testprojekte');
  }
  static async getTestprojekteEntry(id: string): Promise<Testprojekte | undefined> {
    ...
    const record = { record_id: data.id, ...data } as Testprojekte;
    return enrichLookupFields([record], 'testprojekte')[0];
  }
  ...
  static async getTestausfuehrungen(): Promise<Testausfuehrungen[]> {
    ...
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Testausfuehrungen[];
    return enrichLookupFields(records, 'testausfuehrungen');
  }
  static async getTestausfuehrungenEntry(id: string): Promise<Testausfuehrungen | undefined> {
    ...
    const record = { record_id: data.id, ...data } as Testausfuehrungen;
    return enrichLookupFields([record], 'testausfuehrungen')[0];
  }
  ...
  static async getTestfaelle(): Promise<Testfaelle[]> {
    ...
    return enrichLookupFields(records, 'testfaelle');
  }
  static async getTestfaelleEntry(id: string): Promise<Testfaelle | undefined> {
    ...
    const record = { record_id: data.id, ...data } as Testfaelle;
    return enrichLookupFields([record], 'testfaelle')[0];
  }
